...
describe('shared moderation draft RPC adapter', () => {
  it('saves a versioned Candidate section and maps the returned shared draft', async () => {
    ...
    await expect(saveCandidateModerationDraft({ rpc } satisfies ModerationDraftRpcClient, {
      placeId,
  ...
  it.each([
    ...
    await expect(saveCandidateModerationDraft({ rpc }, {
      placeId,
  ...
  it('submits a Candidate rejection with both concurrency tokens and reasons', async () => {
    ...
    await expect(decideCandidatePlace({ rpc }, {
      placeId,
      outcome: 'rejected',
