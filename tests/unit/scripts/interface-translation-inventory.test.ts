...
describe('interface translation inventory release SQL', () => {
  it('renders one environment-safe capability configuration, sync, and verification transaction', () => {
    ...
    expect(sql).not.toContain('production-baseline');
    expect(sql).not.toContain('expected_workspace_revision');
    expect(sql).toContain(":'release_sha'");
    expect(sql).toContain('public.mark_interface_translation_source_candidate_applied()');
